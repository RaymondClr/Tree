...
parseScriptUI({
    rectbutton: [
        ...
        '按钮',
        {
            ...
            fontName: 'Tahoma', //按钮字体
            fontStyle: 'REGULAR', //按钮字体样式
            fontSize: 12, //按钮文字大小
            fontOffset: [0, 0], //按钮文字的偏移坐标
...
- 所有参数都是支持缺省的，另外如果你发现配置了参数之后，按钮的外观并没有发生改变，那么只有两种可能，一是你的参数是无效的，二是你的参数超出了最大限制。为了尽可能得对各位友好一点，我并没有选择在这种时候用弹窗的方式向你展示错误（相信大家都讨厌弹窗，尤其是错误的弹窗），所有非法输入被检测到后，都会使用默认参数进行替补，所以你可以大胆的往参数里写任何内容。
